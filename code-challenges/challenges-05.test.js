...
const toLastNames = people => {
  ...
  let arr = [];
  people.map((word) => {
    arr.push(word.firstName + ' ' + word.lastName);
  });
  return arr;
};
...
const addValues = (arr) => {
  // Solution code here...
  let sum = arr.reduce((acc, elm) => { return acc += elm; }, 0);
  return sum;
};
...
const reversedString = (str) => {
  // Solution code here...
  const arr = str.split('');
  const reversed = arr.reduce((acc, letter) => { return letter + acc; }, '');
  return reversed;
};
...
const extractChildren = (arr) => {
  // Solution code here...
  let filteredArr = arr.filter(elm => elm.name.includes('a'));
  return filteredArr.reduce((acc, elm) => {
    if (elm.children) {
      elm.children.forEach(elm => acc.push(elm));
    }
    return acc;
  }, []);
};
