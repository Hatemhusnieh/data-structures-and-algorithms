...
const typeNum = (arr) => {
  // Solution code here...
  const arr2 = arr.filter(num=> !isNaN(num));
  return arr2;
};
...
const containsAnd = (arr) => {
  // Solution code here...
  const arr2 = arr.filter(elm => elm.includes('and'));
  return arr2;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  // Solution code here...
  const arr2 = arr.filter(elm =>!(forbiddenValues.includes(elm)));
  return arr2;
};
...
const getStatName = (arr, minBaseStat) => {
  ...
  const statsName = stats.map(elm =>elm.stat.name);
  return statsName;
};
...
const evenOddNumericValues = (arr) => {
  // Solution code here...
  const numeric = arr.filter(num => typeof(num) === 'number');
  return numeric.map(num =>num % 2 === 0 ? 'even' : 'odd');
};
